...
res <- spwbpoints(examplepointslandscape, SpParamsMED, examplemeteo, 
                  dates = dates, localControl = control,  progress = FALSE)
```
...
Element `sp` describes spatial information, vector `xlist` contains the `spwbInput` corresponding to each forest stand, vector `resultlist` contains the output of `spwb()` function for each forest stand (i.e. its elements are objects of the S3 class `spwb`) and `summarylist` contains summaries (if requested, see below). 

### Summaries and plots

The structure of the output of `spwbpoints()` allows querying information for the simulation of any particular forest stand. For example, if we want a monthly summary of water balance for the first stand, we can use function `summary.spwb()` from package **medfate**:
...
A more convenient way of generating summaries, though, is by calculating them on all forest stands, using:

```{r}
res_sum <- summary(res, summaryFunction = summary.spwb, 
                   freq="months", output="WaterBalance")
```
where the argument `summaryFunction` points to the function to be used to generate summaries and the remaining arguments are those of the summary function. The result is an object of class:
```{r}
class(res_sum)
```
which contains two elements, the spatial information and the summaries:
```{r}
names(res_sum)
```
The summary for the first stand can be accessed through the first element of `summarylist`:
```{r}
res_sum$summarylist[[1]]
```

Summary objects are handy because they allow us to display plots:
```{r, fig.align='center', fig.width=7}
...
res_2 <- spwbpoints(examplepointslandscape, SpParamsMED, examplemeteo, 
                  dates = dates, localControl = control,                  
                  keepResults = FALSE, progress = FALSE,
                  summaryFunction = summary.spwb, args = list(freq="months"))
```
...
```{r, fig.align='center', fig.width=7}
plot(res_2, "Transpiration", "2001-03-01")
```
