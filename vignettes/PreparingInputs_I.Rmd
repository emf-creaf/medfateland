...
output: 
  rmarkdown::html_vignette:
    toc: TRUE
vignette: >
  %\VignetteIndexEntry{Preparing inputs I: forest inventory plots}
...
```{r}
y_1 <- create_landscape(x, dem = dem, progress = FALSE)
y_1
```

Note that by default, the function `create_landscape()` assumes that are locations are `wildland`. Land cover type was not needed here, but it may be relevant in other target locations/areas.

## Forest inventory parsing
