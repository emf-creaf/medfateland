...
To avoid displaying maps one by one, the package includes function `shinyplot()` that allows displaying maps of temporal summaries interactively.

Finally, summary objects can be reformatted using function `unnest_summary()` to obtain an sf object more amenable for storing and data manipulation:
...
or extract summary results using `unnest_summary()`:
...
In this last case, function `unnest_summary()` will be only helpful if the summary columns have the same names for all cells.
