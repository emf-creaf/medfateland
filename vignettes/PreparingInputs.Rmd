...
```{r}
y_3 <- modify_forest_structure(y_2, height_map_40, var = "mean_tree_height")
```
...
### Evaluating the correction effect
...
p1 <- plot_variable(y_2, "basal_area", r = r)+
  ...
  scale_fill_distiller("m2/ha", limits = c(0,70), palette = "Spectral", na.value = NA)+
...
```{r}
medfate::stand_dominantTreeHeight(y_2$forest[[1]])
medfate::stand_dominantTreeHeight(y_3$forest[[1]])
medfate::stand_dominantTreeHeight(y_4$forest[[1]])
```

or stand basal area:

```{r}
medfate::stand_basalArea(y_2$forest[[1]])
medfate::stand_basalArea(y_3$forest[[1]])
medfate::stand_basalArea(y_4$forest[[1]])
```
...
p1 <- plot_variable(y_2, "mean_tree_height", r = r)+
  scale_fill_distiller("cm", limits = c(0,2600), palette = "Spectral", na.value = NA)+
  geom_spatvector(fill = NA, col = "black", linewidth = 0.5, data = watershed)+
...
p4 <- ggplot()+
  geom_spatraster(aes(fill=height), data=height_map_40)+
  geom_spatvector(fill = NA, col = "black", linewidth = 0.5, data = watershed)+
  scale_fill_distiller("m", limits = c(0,26), palette = "Spectral", na.value = NA)+
...
```{r}
mth_2 <- extract_variables(y_2, "mean_tree_height")$mean_tree_height
mth_3 <- extract_variables(y_3, "mean_tree_height")$mean_tree_height
mth_4 <- extract_variables(y_4, "mean_tree_height")$mean_tree_height

x_vect <- terra::vect(sf::st_transform(sf::st_geometry(x), terra::crs(height_map_40)))
x_mth<-terra::extract(height_map_40, x_vect)[,-1, drop = TRUE]

cor.test(mth_2, x_mth)
cor.test(mth_3, x_mth)
cor.test(mth_4, x_mth)
```
