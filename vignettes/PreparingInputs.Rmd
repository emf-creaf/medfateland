...
Now we define a raster at 100 m resolution, including the target area. We intersect it with the watershed boundaries to keep the target locations:

```{r, warning = FALSE}
res <- 300
r <-terra::rast(terra::ext(watershed), resolution = c(300,300), crs = "epsg:25831")
...
The number of cells for each land cover type is the following:
...
This resolution is finer than the size of forest inventory plots, and the errors in the plot coordinates may lead to missing height data (the map only deals with areas with forest cover). Hence, we aggregate the raster to the 40m resolution:
...
```{r}
height_r <- terra::crop(height_map_40, r)
names(height_r)<- "height"
```


```{r}
ggplot()+
  geom_spatraster(aes(fill=height), data=height_r)+
  geom_spatvector(fill = NA, col = "black", linewidth = 1, data = watershed)+
  scale_fill_continuous("m", type = "viridis", na.value = NA)+
  theme_bw()
```

```{r}
y_3 <- modify_forest_structure(y_2, height_map_40, var = "mean tree height")
```
...
y_3 <- impute_forests(y_1, sf_nfi = sf_nfi, dem = dem, 
                      forest_map = forest_map, height_map = height_map_40)
```
...
Additionally, one may have access to other maps of structural variables. In our case, we will use a raster of tree density, also derived from LiDAR flights:
...
```{r}
density_map_40 <- terra::aggregate(density_map, fact = 2, fun = "mean", na.rm = TRUE)
```


```{r}
density_r <- terra::crop(density_map_40, r)
names(density_r)<- "N"
```
...
```{r}
ggplot()+
  geom_spatraster(aes(fill=N), data=density_r)+
  geom_spatvector(fill = NA, col = "black", linewidth = 1, data = watershed)+
  scale_fill_continuous("ind/ha", type="viridis", na.value = NA)+
  theme_bw()
```

```{r}
y_4 <- modify_forest_structure(y_3, density_map_40, var = "tree density")
```
...
y_4 <- impute_forests(y_1, sf_nfi = sf_nfi, dem = dem, 
                      forest_map = forest_map, height_map = height_map_40, density_map = density_map_40)
```

Analogously to the correction of heights, the correction for tree density does not take the tree density value of the map for the target location, but the ratio of values between the target location and the forest inventory plot used as reference for imputation. We can inspect the same forest again to see the changes in `N` (density per hectare) column:
...
```{r}
medfate::stand_dominantTreeHeight(y_2$forest[[11]])
medfate::stand_dominantTreeHeight(y_3$forest[[11]])
medfate::stand_dominantTreeHeight(y_4$forest[[11]])
```

or stand basal area:

```{r}
medfate::stand_basalArea(y_2$forest[[11]])
medfate::stand_basalArea(y_3$forest[[11]])
medfate::stand_basalArea(y_4$forest[[11]])
```
...
```{r}
basal_area_map_40 <- terra::aggregate(basal_area_map, fact = 2, fun = "mean", na.rm = TRUE)
```


```{r}
basal_area_r <- terra::crop(basal_area_map_40, r)
names(basal_area_r)<- "basal_area"
```


```{r, echo = TRUE, fig.width=8, fig.height=6, warning = FALSE, message = TRUE}
p1 <- plot_variable(y_2, "basal_area", r = r)+
  scale_fill_distiller(limits = c(0,120), palette = "Spectral", na.value = NA)+
  labs(title = "Imputation alone")
p2 <- plot_variable(y_3, "basal_area", r = r)+
  scale_fill_distiller(limits = c(0,120), palette = "Spectral", na.value = NA)+
  labs(title = "Imputation and height correction")
p3 <- plot_variable(y_4, "basal_area", r = r)+
  scale_fill_distiller(limits = c(0,120), palette = "Spectral", na.value = NA)+
  labs(title = "Imputation and height + density correction")

p4 <- ggplot()+
  geom_spatraster(aes(fill=basal_area), data=basal_area_r)+
  geom_spatvector(fill = NA, col = "black", linewidth = 1, data = watershed)+
  scale_fill_distiller(limits = c(0,120), palette = "Spectral", na.value = NA)+
  theme_bw()
...
```
```{r}
ba_2 <- extract_variables(y_2, "basal_area")$basal_area
ba_3 <- extract_variables(y_3, "basal_area")$basal_area
...
x_ba<-terra::extract(basal_area_map_40, x_vect)[,-1, drop = TRUE]
