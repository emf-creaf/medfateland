...
First we load a polygon data set describing watersheds in Spain and select our target watershed (Riera de Bianya):
```{r}
...
We now call function `modify_forest_structure()` to correct mean tree height according to the LiDAR data (where vegetation height should be in meters):
...
We now use the same function to correct basal area values. 
...
```
where it is apparent that both the height correction and the basal area correction have an effect in basal area. We can quantitatively assess the relationship between predicted basal area and the observed one using:
