...
```{r}
x <- sf::st_geometry(sf::st_as_sf(v))
x
```
...
```{r, echo = TRUE, fig.width=8, fig.height=12, warning = FALSE, message = TRUE}
...
```{r}
y_8$bedrock_porosity <- pmax(0.001, x_glhymps[,"Porosity_x", drop = TRUE]/100)
```

We force a minimum of 0.1% porosity to avoid numerical problems during simulations. Its geographic distribution is quite simple:
...
```{r}
r$value <- TRUE
terra::writeRaster(r, "bianya_raster.tif")
```
