...
Despite the geometries themselves (which have a coordinate reference system), the following columns are required:
  ...
  + `elevation`, `slope`, `aspect` describe the topography of the stands
...
In **medfateland** there are several alternatives to specify climate forcing. One simple option is to assume the same weather for all stands. In this case we will supply a single data frame with daily weather for all plots:
