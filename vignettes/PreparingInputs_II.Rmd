...
output: 
  rmarkdown::html_vignette:
    toc: TRUE
vignette: >
  %\VignetteIndexEntry{Preparing inputs II: arbitrary locations}
...
Having these inputs, we can use functions `add_topography()` and `add_land_cover()` to add topographic and land cover features to our starting `sf`:
```{r}
y_1 <- add_topography(x, dem = dem)
y_1 <- add_land_cover(y_1, 
                      land_cover_map = lcm, 
                      wildland = wildland, 
