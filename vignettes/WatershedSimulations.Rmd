...
Note that the `sf` structure does not imply a grid *per se*. Point geometry is used to describe the central coordinates of grid cells, and `represented_area_m2` is used to define the area of cells in square meters (all values should be the same!).
...
res_ws <- spwb_land(r, example_watershed, SpParamsMED, examplemeteo, dates = dates, summary_frequency = "month")
```
