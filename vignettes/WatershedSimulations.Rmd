...
Here we use function `defaultControl()` to specify the control parameters for local processes. Function `initialize_landscape()` makes internal calls to `forest2spwbInput()` of **medfate** and defines a column `state` with the initialized inputs.
