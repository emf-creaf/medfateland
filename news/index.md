...
- New function unnest_summary() to extract cell summaries as unnested
  sf.

## medfateland 2.8.2

- Backwards compatibility of objects initialized with medfate ver. 4.8.3
  and 4.8.4
- Catching spwb_day and growth_day errors in simulations of
  spwb_spatial_day() and growth_spatial_day()
...
- Negative aquifer correction introduced to detect when closing local
  water balance induces undue capillarity from the aquifer
- Modification of vertical partitioning of interflow to alleviate the
  need of aquifer correction
...
- Subwatersheds returned as part of spatial output in \*\_land
  simulations
- Bug correction for .check_meteo_column_input issue
  [\#5](https://github.com/emf-creaf/medfateland/issues/5)
...
- New channel routing algorithm (variable velocity) suggested by M.
  Gonzalez-Sanchis
...
- Option `simplify` in function initialize_landscape() replaced by
  `reduce_to_dominant`, `merge_trees` and `merge_shrubs`
...
- information on soil domain and transpiration mode of initialized
  landscapes
- overland routing available in new function overland_routing()
- overland flow allowed in completely flat areas (according to
  processing order)
...
- Bug correction: input column regen is no longer kept in
  parse_forestable()
...
- Input sf can contain column ‘local_control’ to override control for
  specific cells/locations.
...
- Fire regimes can be simulated in functions fordyn_spatial and
  fordyn_scenario
...
- IMPORTANT: In this version we removed sp-type S4 classes that were
  deprecated in 1.0.1. Use release version 1.2.1 to continue using
  deprecated classes.
...
- New plotting functions ‘plot.summarypoints’, ‘plot.summarypixels’ and
  ‘plot.summarygrid’
...
- Parallelization in spwb and growth simulations with Spatial\*Landscape
  objects
